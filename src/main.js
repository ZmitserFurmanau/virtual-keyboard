...
class Keyboard {
  ...
  init() {
    ...
    for (const [key, value] of Object.entries(this.keys)) {
      ...
      const { keyMain, keySecond, keyMainBy, keySecondBy, classes } = params;
  ...
  getKeyParams(key) {
    ...
    if (this.keys[key].en && this.keys[key].by) {
      ...
      params.keySecond =
        params.keyMain === this.keys[key].en.shiftKey
          ? ""
          : this.keys[key].en.shiftKey;
      params.keyMainBy = keys[key].by.key.toUpperCase();
      params.keySecondBy =
        params.keyMainBy === this.keys[key].by.shiftKey
          ? ""
    ...
    if (key === "ShiftLeft" || key === "ShiftRight")
      params.classes.push("key_shift");
    if (key === "ControlRight" || key === "ControlLeft")
      params.classes.push("key_ctrl");
  ...
  keyPressHandler(key) {
    ...
    if (key === "ArrowUp") {
      this.cursorPosition =
        this.cursorPosition - 42 > 0 ? this.cursorPosition - 42 : 0;
      this.textarea.setSelectionRange(this.cursorPosition, this.cursorPosition);
      return;
    }
    if (key === "ArrowDown") {
      this.cursorPosition =
        this.cursorPosition + 42 < this.textarea.value.length
  ...
  switchLayout() {
    ...
    keysArr.forEach((key) =>
      [...key.querySelectorAll("span")].forEach((span) =>
        span.classList.toggle("key_invisible")
      )
    );
    localStorage.setItem("lang", this.state.lang);
    document.querySelector(".button_layout").textContent =
  ...
  getContent(key) {
    ...
    if (this.state.isShiftPressed)
      return this.keys[key][this.state.lang].shiftKey;
